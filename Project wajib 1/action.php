...
if(isset($_POST['add'])){
  ...
  $query="INSERT INTO mahasiswa(name,jk,prodi,fakultas,image)VALUES(?,?,?,?,?)";
...
if(isset($_GET['delete'])){
  ...
  $sql="SELECT image FROM mahasiswa WHERE id=?";
...
if(isset($_POST['update'])){
  ...
  $query="UPDATE mahasiswa SET name=?,jk=?,prodi=?,fakultas=?,image=? WHERE id=?";
  $stmt=$conn->prepare($query);
...
if(isset($_GET['cari'])){
  $cari=$_GET['cari'];
  $query = "SELECT * FROM mahasiswa where name LIKE '%".$cari."%'";
  $stmt=$conn->prepare($query);
