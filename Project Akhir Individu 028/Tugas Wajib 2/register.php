...
if(isset($_POST['reg'])){
    ...
    try {
        $query="INSERT INTO loginphp(fullname,email,username,password,type)VALUES(?,?,?,?,?)";
        $stmt=$conn->prepare($query);
        $stmt->bind_param("sssss",$fn,$em,$un,$pw,$tp);
        $stmt->execute();
        if($stmt->execute()){
            $to      = $em;
            ...
            http://127.0.0.1:3307/login_uas/loginphp'.$em.'&hash='.$hash.'
...
<div class="panel panel-default" style="margin-top:80px;">
    <div class="panel-body">
        <h3 class="text-center" style="font-weight:bolder;margin:10px 0; color:orange;">User Register</h3>
        <form method="post">
            ...
            <button type="submit" name="reg" class="btn btn-warning btn-block">Register</button>
            <p style="clear:both;">
